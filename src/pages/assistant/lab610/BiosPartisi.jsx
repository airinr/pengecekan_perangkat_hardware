...
// === KONFIG KODE ===
const LAB_CODE = "LAB00002"; // kodelab
const PRAKTIKUM_CODE = "PRTK00002"; // kode praktikum
...
// Kolom disamakan + Asisten + Status + Aksi
const columns = [
  { key: "no", header: "#" },
...
// helper normalisasi URL TTD (absolute/relative)
const toUrl = (v) => {
...
export default function BiosPartisi() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        // sertakan ?lab=LAB00001 (tetap difilter di client untuk aman)
        const url = new URL(
        ...
        const mapped = list.map((h, i) => {
          ...
          const goDetail = () => {
            ...
            navigate(`/history_detail/${encodeURIComponent(hid)}`, {
              state: { item: h }, // opsional: lempar payload asli
            });
          };
  ...
  // Kirim kode praktikum + lab sebagai query ke halaman form
  const addHref = useMemo(() => {
    const q = new URLSearchParams({ kode: PRAKTIKUM_CODE, lab: LAB_CODE });
    return `/form_bios_partisi?${q.toString()}`;
  }, []);

  return (
    <TablePage
      title="Perangkat Laboratorium 610 - Bios dan Partisi"
      addRoute={addHref}
