...
export default function FormMerakitPc() {
  // Ambil dari query: ?kode=PRTK00002&lab=LAB00001 (contoh)
  ...
  const [header, setHeader] = useState({
    ...
    dosen: "",
    kelas: "",
    asisten: getCurrentUserName(), // AUTO dari user
    waktuMulai: "", // dikirim sebagai "waktu"
    waktuSelesai: "",
  });
  ...
  const [rows, setRows] = useState(
    DEFAULT_ITEMS.map((name, i) => ({
      ...
      awal: "", // diisi dari datalab (jumlahNormal + jumlahRusak)
      akhir: "",
      kerusakan: "",
    }))
  );
  ...
  // Foto praktikum (samakan field dgn merakit: "photoPraktikum")
  ...
  // Prefill dari /datalab/getdatalab/{lab} -> isi idBarang & jumlah awal
  const fetchDatalab = useCallback(async () => {
    ...
    try {
      ...
      // Map namaBarang(lowercase) -> { idBarang, awal (normal+rusak), normalOnly }
      ...
      list.forEach((it) => {
        ...
        if (nama && idBarang) nameToInfo.set(nama, { idBarang, awal, jn, jr });
      });

      // Prefill baris berdasarkan nama item di form
      setRows((prev) =>
        prev.map((r) => {
          ...
          return {
            ...r,
            idBarang: info.idBarang,
            // Jika ingin mengambil AWAL dari jumlahNormal saja (sesuai permintaan terakhir),
            // ganti 'info.awal' menjadi 'info.jn' di bawah.
            awal: info.jn, // jumlahNormal sebagai jumlah awal
          };
        })
      );
    } catch (err) {
  ...
  // Sinkronkan asisten dari localStorage saat mount (jika state kosong)
  useEffect(() => {
  ...
  const onSubmit = async (e) => {
    ...
    fd.append("asisten", header.asisten || getCurrentUserName()); // <- ikut kirim
