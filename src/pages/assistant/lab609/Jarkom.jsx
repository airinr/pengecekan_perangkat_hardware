// src/pages/JaringanKomputer.jsx
...
// ====== KONFIG PARAM ======
const LAB_CODE = "LAB00001"; // kodelab 610?
const PRAKTIKUM_CODE = "PRTK00003"; // kode praktikum Jaringan Komputer
...
// Kolom yang tersedia dari API history (+ TTD) + Status + Aksi
const columns = [
  { key: "no", header: "#" },
...
export default function JaringanKomputer() {
  ...
  // Ambil histori praktikum Jarkom
  useEffect(() => {
    let alive = true;
    (async () => {
      setLoading(true);
      ...
      try {
        ...
        // sertakan ?lab=LAB00001 (tetap difilter di client)
        const url = new URL(
  ...
  const addHref = useMemo(() => {
    ...
    return `/form_jarkom?${q.toString()}`;
  }, []);

  return (
    <TablePage
      title="Perangkat Laboratorium 609 - Jaringan Komputer"
      addRoute={addHref}
